		...
		<div className="container w-50">
			...
			<div className="d-flex col-md-12 py-1 bg-dark text-light text-center sticky-bottom">
				<div className="btn-group m-auto" role="group" aria-label="Basic example">
					<button onClick={() => prevsong(songRef.current.id)} type="button" className="btn btn-secondary">
						<i className="mx-2 fa-solid fa-backward-step"></i>
					</button>
					<button onClick={() => playorpause()} type="button" className="btn btn-secondary">
						{playing === false ?
							<i className="mx-2 fa-solid fa-play"></i> :
							<i className="fa-solid fa-pause"></i>
						}
					</button>
					<button onClick={() => nextsong(songRef.current.id)} type="button" className="btn btn-secondary">
						<i className="mx-2 fa-solid fa-forward-step"></i>
					</button>
					<button onClick={() => loop()} type="button" className="btn btn-secondary">
						<i className="fa-solid fa-repeat"></i>
					</button>
					<button onClick={() => shuffle()} type="button" className="btn btn-secondary">
						<i className="fa-solid fa-shuffle"></i>
					</button>
				</div>
				<div className="btn-group m-auto" role="group" aria-label="Basic example">
					<button onClick={() => volumedown()} type="button" className="ms-1 btn btn-secondary">
						<i className="fa-solid fa-minus"></i>
					</button>
					<button className="btn btn-secondary disabled">Volume</button>
					<button onClick={() => volumeup()} type="button" className="btn btn-secondary">
